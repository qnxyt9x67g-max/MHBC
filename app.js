...
function initFirebase() {
  ...
  // App Check (reCAPTCHA v3). Keep Console in Monitor mode until metrics look
  // healthy; enforce later. Activate before other services use the network.
  try {
...
function checkApprovalAndEnter(attempt) {
  attempt = attempt || 0;
...
window.onload = function () {
  ...
  auth.onAuthStateChanged(function (user) {
    if (user) {
      ...
      function applyCachedSessionVars() {
        var lastGroup = getLastGroup();
