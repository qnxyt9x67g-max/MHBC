...
// Hide input bar when leaving care page
if (id !== ‘care’) {
    ...
    memberRef.get().then(function(memberSnap) {
      if (memberSnap.exists) {
        ...
        hashInput(userPin, data.pinSalt).then(function(enteredPinHash) {
          if (enteredPinHash !== data.pinHash) { errEl.textContent = 'Incorrect PIN for that name. Try again.'; return; }
          // Update isAdmin in case they're logging in as admin on existing account
          ...
          if (data.approved || isAdmin) { enterChat(); }
...
```
  ...
  topLevel.forEach(function(msg) {
    renderMessage(msg, messagesEl, snapshot, false);
  });
