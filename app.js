...
<head>
<meta charset="UTF-8">
<title>app.js — Copy to GitHub</title>
<style>
  body { margin: 0; background: #0a1628; color: #e8e0d0; font-family: monospace; }
  .toolbar { position: sticky; top: 0; background: #1a2a44; padding: 12px 16px; display: flex; align-items: center; gap: 12px; border-bottom: 1px solid #c9a84c; z-index: 10; }
  .toolbar h2 { margin: 0; color: #c9a84c; font-size: 15px; flex: 1; }
  .copy-btn { background: #c9a84c; color: #0a1628; border: none; padding: 8px 20px; border-radius: 6px; font-weight: 700; font-size: 14px; cursor: pointer; }
  .copy-btn:hover { background: #e0c060; }
  .copy-btn.copied { background: #4caf50; color: white; }
  pre { margin: 0; padding: 20px; white-space: pre-wrap; word-break: break-word; font-size: 12px; line-height: 1.5; }
</style>
</head>
<body>
<div class="toolbar">
  <h2>app.js v21 final — Copy everything below into GitHub</h2>
          ...
          if (memberSnap.exists) {
            ...
            memberRef.set({
              ...
              if (restoredApproved || restoredIsAdmin) {
                return memberRef.update({ approved: restoredApproved, isAdmin: restoredIsAdmin });
              }
            }).then(function() {
              ...
              var deletes = [];
              oldSnap.forEach(function(d) { if (d.id !== currentUID) deletes.push(d.ref.delete()); });
